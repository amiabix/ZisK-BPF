...
fn main() -> Result<()> {
    ...
    println!("📁 [BPF-EXECUTOR] Using BPF program: {}", program_path);
    
    // Check if the program file exists
    if !fs::metadata(program_path).is_ok() {
        eprintln!("❌ [BPF-EXECUTOR] Program file not found: {}", program_path);
        std::process::exit(1);
    }
    
    // Read the program data
    let program_data = fs::read(program_path)?;
    println!("✅ [BPF-EXECUTOR] Loaded program: {} bytes", program_data.len());
    ...
    if let Ok(output_data) = bincode::serialize(&execution_result) {
        ...
        if let Ok(_) = fs::write(output_file, &output_data) {
            ...
            // Print summary
            println!("📊 [BPF-EXECUTOR] Execution Summary:");
            ...
            eprintln!("❌ [BPF-EXECUTOR] Failed to write output file");
            std::process::exit(1);
        }
    } else {
        eprintln!("❌ [BPF-EXECUTOR] Failed to serialize execution result");
        std::process::exit(1);
    }
    
    println!("🎉 [BPF-EXECUTOR] Real BPF execution completed successfully!");
    Ok(())
}
